...
% sby = y coordinate of the s-BS
 sinrt=-5;   % predefind SINR threshold value
% tp= transmission power of
% smax= mxmimum number ues supported by bs 
smax=30;
%pa= Circuit power of active s-BS
%ps= Circuit power of sleep s-BS

ues=xlsread('G:\kvr\lockdown\oct-2020 onwards papers\wpc per for conferne updated one\code\WPCPoints','CUES');
uex=ues(:,1);  %intailzation points
 uey=ues(:,2);
 %range=250;
sue=size(ues,1);
 z=1;
sbs=xlsread('G:\kvr\lockdown\oct-2020 onwards papers\wpc per for conferne updated one\code\WPCPoints','CBS');
ssb=size(sbs,1);
 sbsx=sbs(:,1);
 sbsy=sbs(:,2);
 ...
 uel=0;
 while(yes==1)
     [ssb,bsleast,sbsx,sbsy,Alluesconn,uelr,yes]=connb(uex,uey,sbsx,sbsy,ssb,sue,srtpm,uel,z);
   ...
   uel=[sbsx,sbsy];
     ...
     [ssb,xss,yss]=rbase(bsleast,sbsx,sbsy,ssb);
sbsx=0;sbsy=0;
     sbsx=xss(:);
sbsy=yss(:);
...
q=0;
 ...
 function [ssb,bsleast,sbsx,sbsy,Alluesconn,uelr,yes]=connb(uex,uey,sbsx,sbsy,ssb,sue,srtpm,uel,z)
sinrt=-5;
smax=30; 
for i=1:ssb
    for j=1:sue        
        ...
        snr(i,j)=(((1-30)*(10*log10(output(i,j)^-1.5)-30))/(-97-30)); %SINR caculation    
...
for i=1:ssb    
    bsloads(i)=bsdegree(i)/smax;     % calculate the each bs
    if(bsloads(i)>0)
        bsactive(i)=1;
    else
         bsactive(i)=0;
...
%%EE
pa=39-30;%
 ps=35-30;%convert dbm to db 35-30
sm=30;
SNT=0;
NC=0;
ussb=ssb-1;
...
NC=NC+(bsactive(c)*((((40-30)*F_c)/sm)+pa))+((1-bsactive(c))*ps);    
...
figure(z);
 ...
 plot(uex,uey,'bo','MarkerSize',5,'LineWidth',1,'MarkerFaceColor',[0.5,0.5,0.5]); 
 hold on;   
 grid on;
 plot(sbsx,sbsy,'r^','MarkerSize',7,'LineWidth',1,'MarkerFaceColor','r');
 A=xlsread('G:\kvr\lockdown\oct-2020 onwards papers\wpc per for conferne updated one\code\WPCPoints','CBS');
 B=[sbsx,sbsy];
 [finalsx,finalsy]=ismember(A,B,'rows'); 
 A(finalsx, :)=[];
 plot(A(:,1),A(:,2),'g^','MarkerSize',7,'LineWidth',1,'MarkerFaceColor','k');
hold on
xlabel('Distance');
ylabel('Distance');
legend('UE','s-BS');
...
for j=1:sue
if (Assign(2,j)==i)  
 line([uex(j),sbsx(i)], [uey(j), sbsy(i)], 'Color', 'r');   
  hold on;   
end
